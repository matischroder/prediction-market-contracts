...
async function main() {
    ...
    const isSepoliaFork = network.name === "hardhat" && networkConfig.forking?.url?.includes("sepolia");
    ...
    let shouldDeployNostronet = true;
    ...
    if (isSepoliaFork || isRealSepolia) {
        ...
        CHAINLINK_CONFIG = {
            ...
            KEY_HASH: "0x474e34a077df58807dbe9c96d3c009b23b3c6d0cce433e59bbf5b34f823bc56c", // Sepolia 30 gwei key hash
        ...
        CHAINLINK_CONFIG = {
            ...
            KEY_HASH: "0x6c3699283bda56ad74f6b855546325b68d482e983852a7a82979cc4807b641f4", // Any valid hash
    ...
    // Deploy MockERC20 for testing (Nostronet/NOS) - solo si no estamos en red real
    let usdcAddress;
    let usdc; // Declarar la variable usdc aquí
    if (shouldDeployNostronet) {
        console.log("\n📄 Deploying Nostronet (NOS)...");
        const MockERC20 = await ethers.getContractFactory("MockERC20");
        usdc = await MockERC20.deploy("Nostronet", "NOS", 6);
        await usdc.waitForDeployment();
        usdcAddress = await usdc.getAddress();
        console.log("✅ Nostronet (NOS) deployed to:", usdcAddress);
    } else {
        // En red real, usar USDC real de Sepolia si existe o deploy mock
        console.log("\n📄 Using Nostronet (NOS) for Sepolia testing...");
        const MockERC20 = await ethers.getContractFactory("MockERC20");
        usdc = await MockERC20.deploy("Nostronet", "NOS", 6);
        await usdc.waitForDeployment();
        usdcAddress = await usdc.getAddress();
    ...
    if (isSepoliaFork) {
        ...
        console.log("   This is a fork of Sepolia, so real LINK contracts exist but we may not have LINK tokens");
        ...
        console.log("   1. Get Sepolia LINK tokens from faucet: https://faucets.chain.link/");
        ...
        console.log("   In localhost mode, VRF calls will fail but deploy succeeds");
        console.log("   This is for contract deployment testing only");
    }

    // Mint some USDC to deployer for testing
    console.log("\n🪙 Minting test NOS tokens...");
    const mintAmount = ethers.parseUnits("10000", 6); // 10,000 NOS
    await usdc.mint(deployer.address, mintAmount);
    console.log("✅ Minted 10,000 NOS to deployer"); // Create a test market (commented out until LINK funding is resolved)
    ...
    console.log("   Nostronet (NOS):", usdcAddress);
    ...
    if (isSepoliaFork) {
        ...
        console.log("2. Get NOS: await usdc.mint(yourAddress, amount)");
        ...
        console.log("1. Complete VRF subscription setup (see LINK funding section above)");
        ...
        console.log("1. Update frontend .env with these addresses");
        console.log("2. Get NOS: await usdc.mint(yourAddress, amount)");
        console.log("3. Test basic functionality (price feeds and VRF will be mocked)");
        console.log("4. Run 'npx hardhat run scripts/deploy.ts --network sepolia' for real testing");
    }

    console.log("\n📝 Available Commands:");
    console.log("   Sepolia fork (recommended): npx hardhat run scripts/deploy.ts --network hardhat");
    console.log("   Pure localhost: Use hardhat.config.local.ts + npx hardhat run scripts/deploy.ts --network hardhat");
    console.log("   Real Sepolia: npx hardhat run scripts/deploy.ts --network sepolia");
    console.log("\n📖 See DEPLOYMENT.md for complete guide and use cases");

    // Save addresses to JSON file
    const networkType = isSepoliaFork ? "sepolia-fork" : isRealSepolia ? "sepolia" : "localhost";
    const addresses = {
        network: networkType,
        chainId: network.config.chainId,
        deployedAt: new Date().toISOString(),
        contracts: {
            nostronet: usdcAddress,
